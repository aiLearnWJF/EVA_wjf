...
def get_list_dst(log_path = [""], list_index_name = [], split_char = [], strip_char = [], \
                 coff_index = [], line_coff = [], ylims = [], legend_labels = [], print_tb = 0):
    ...
    for log_path_index,log_path_i in enumerate(log_path):
        ...
        plot_list(list_index_value, list_index_name, ylims, label = label_legend, title = list_index_name)
        if print_tb:
...
def plot_list(list_dst = [],list_type = [], ylims = [], label = "", title = ""):
    NUMS = len(list_type)
    ...
    for i in range(NUMS):
        ...
        plt.plot(range(len(list_dst[i])), list_dst[i], label = label)
        plt.title(title[i])
...
if __name__=="__main__":
    ...
    # 传入多个值的列表，必须格式一致
    get_list_dst(args.log_path, list_index_name, split_char, strip_char, coff_index, line_coff, ylims, legend_labels, print_tb)
